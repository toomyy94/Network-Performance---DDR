...
#~ #Exponential
#~ exp=np.random.exponential(M,(300,20))
#~ #Gaussian/Normal
#~ gauss=np.random.normal(M,V**0.5,(300,20))
#~ #Empirical discrete
#~ discrt=np.random.choice(bins[:-1],(300,20),p=probs)
#~ 
#~ plt.figure(181)
#~ plt.plot(range(0,300,1), x, marker='s', c="blue")
#~ plt.show()
#~ plt.figure(182)
#~ plt.plot(range(0,300,1), exp, marker='s', c="red")
#~ plt.show()
#~ plt.figure(183)
#~ plt.plot(range(0,300,1), gauss, marker='s', c="green")
#~ plt.show()
#~ plt.figure(184)
#~ plt.plot(range(0,300,1), discrt, marker='s', c="black")
#~ plt.show()
#~ waitforEnter()
#~ exp,binsexp=np.histogram(exp,bins=20)
#~ NO,binsNO=np.histogram(gauss,bins=20)
#~ CH,binsCH=np.histogram(discrt,bins=20)
#~ 
#~ plt.figure(185)
#~ plt.plot(bins[:-1], H, marker='s', c="blue")
#~ plt.plot(binsexp[:-1], exp, marker='s', c="red")
...
if ant == 0:
    ...
    n_ones.append(now)
M_n_ones=np.mean(n_ones)
M_n_zeros=np.mean(n_zeros)
exp_n_ones=np.random.exponential(M_n_ones,(300,20))
exp_n_zeros=np.random.exponential(M_n_zeros,(300,20))
plt.figure(191)
plt.plot(range(0,300,1), exp_n_ones, marker='s', c="blue")
plt.show()
plt.figure(192)
plt.plot(range(0,300,1), exp_n_zeros, marker='s', c="red")
plt.show()
H,bins=np.histogram(x,bins=20)
probs=1.0*H/np.sum(H)
discrt=np.random.choice(bins[:-1],(300,20),p=probs)
plt.figure(193)
plt.plot(range(0,300,1), discrt, marker='s', c="green")
plt.show()
...
ant = 1
cur = 0
length = int(np.random.exponential(M_n_ones,(1,1))[0][0])
i = 0
while (len(traf) != 300):
    if ant == 1:
        if cur < length:
            ...
            cur = 0
            ant = 0
            length = int(np.random.exponential(M_n_zeros,(1,1))[0][0])
    elif ant == 0:
        if cur < length:
            traf.append(0)
            cur += 1
...
plt.figure(196)
plt.plot(H,marker='s',c='blue',label='Original PDF')
plt.plot(H_sint,marker='s',c='red',label='PDF Generated')
plt.show()
